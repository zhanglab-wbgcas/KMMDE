...
setwd('/home/liukc/project/sdc/02func_algori/revise_dre/timedata_pro/100runs/plot_pro/no_deseq/with_impulse')
...
for(replicate.num in replicate.range){
  for(tp_num in tp.range){
    ...
    # totRejection_deseq<- Power_deseq <- tp_deseq<-tn_deseq<-fn_deseq<-fp_deseq<-TPR_deseq <-TNR_deseq <- FNR_deseq <- FDR_deseq <-acc_deseq<-pre_deseq<-recall_deseq<-spec_deseq<-f1_deseq<-matrix(NA, nrow=length(deg.range), ncol=total)
    ...
    totRejection_impulsede2<- Power_impulsede2 <- tp_impulsede2<-tn_impulsede2<-fn_impulsede2<-fp_impulsede2<-TPR_impulsede2 <-TNR_impulsede2 <- FNR_impulsede2 <- FDR_impulsede2 <-acc_impulsede2<-pre_impulsede2<-recall_impulsede2<-spec_impulsede2<-f1_impulsede2<-matrix(NA, nrow=length(deg.range), ncol=total)
    ...
    for(deg_num in deg.range){
      ...
      while (num <= total){
        ...
        header <- paste(rep(((1:tp_num)-1)*3,each=as.integer(replicate.num)),"h-",rep(1:as.integer(replicate.num)),sep="")
        time <- ((1:tp_num)-1)*3
        type <- c("control","case")
        ...
        design <- data.frame("Sample"=colnames(count),
                             "Condition"=rep(type,each=tp_num*replicate.num),
                             "Time"=rep(rep(time,each=replicate.num),2),
        ...
        impulsede2_res <- impulse_results$dfImpulseDE2Results[,c(1,3)]
        
        ms_gene <- impulsede2_res$Gene
        Genes <- rownames(count)
        del_gene <- Genes[!Genes %in% ms_gene]
        length(del_gene)
        new_genedf <- data.frame('Gene'=del_gene, 'padj' = rep(1, length(del_gene)) ,check.names=FALSE)
        #
        ...
        print(tail(impulsede2_res))
        
        
        TP <- dim(impulsede2_res[as.numeric(impulsede2_res$DEG)==0 & impulsede2_res$padj <= fdr_thres, ])[1]
        FN <- dim(impulsede2_res[as.numeric(impulsede2_res$DEG)==0 & impulsede2_res$padj > fdr_thres, ])[1]
        FP <- dim(impulsede2_res[as.numeric(impulsede2_res$DEG)==1 & impulsede2_res$padj <= fdr_thres, ])[1]
        TN <- dim(impulsede2_res[as.numeric(impulsede2_res$DEG)==1 & impulsede2_res$padj > fdr_thres, ])[1]
        TPR <- TP / (TP + FN)
        TNR <- TN / (TN + FP)
        FNR <- FN/(TP+FN)
        FDR <- FP/(TP+FP)
        ...
        specificity <- TN/(TN + FP)
        f1 <- 2/((1/precision)+(1/recall))
        totRejection <- sum(impulsede2_res$padj <= fdr_thres, na.rm=TRUE)
        Power <- sum(impulsede2_res$padj[1:deg_num] <= fdr_thres, na.rm=TRUE)/deg_num
        totRejection_impulsede2[row_num, num] <- totRejection
        Power_impulsede2[row_num, num] <-Power
        print(f1);print(TP);print(FDR);print(FNR);print(totRejection);print(Power);print(accuracy)
        
        tp_impulsede2[row_num, num]<-TP
        tn_impulsede2[row_num, num]<-TN
        fn_impulsede2[row_num, num]<-FN
        fp_impulsede2[row_num, num]<-FP
        acc_impulsede2[row_num, num] <- accuracy
        recall_impulsede2[row_num, num] <- recall
        spec_impulsede2[row_num, num] <- specificity
        pre_impulsede2[row_num, num] <- precision
        f1_impulsede2[row_num, num] <- f1
        FNR_impulsede2[row_num, num] <- FNR
        FDR_impulsede2[row_num, num] <- FDR
        TPR_impulsede2[row_num, num] <- TPR
        TNR_impulsede2[row_num, num] <- TNR
      ...
      write.csv(FNR_impulsede2, file=paste("FNR_impulsede2",replicate.num,tp_num,"res.csv",sep="_"))
      write.csv(FDR_impulsede2, file=paste("FDR_impulsede2",replicate.num,tp_num,"res.csv",sep="_"))
      write.csv(f1_impulsede2, file=paste("f1_impulsede2",replicate.num,tp_num,"res.csv",sep="_"))
      write.csv(totRejection_impulsede2,file=paste("totRejection_impulsede2",replicate.num,tp_num,"res.csv",sep="_"))
      write.csv(Power_impulsede2,file=paste("Power_impulsede2",replicate.num,tp_num,"res.csv",sep="_"))
    ...
    vartotRejection_impulsede2 <- meantotRejection_impulsede2 <- meanPower_impulsede2 <- meantp_impulsede2 <- meantn_impulsede2 <-meanfn_impulsede2 <-meanfp_impulsede2 <- meanTPR_impulsede2 <- meanTNR_impulsede2 <-meanFNR_impulsede2 <- meanFDR_impulsede2 <- meanacc_impulsede2  <- meanpre_impulsede2 <- meanrecall_impulsede2 <- meanspec_impulsede2 <- meanf1_impulsede2 <-rep(NA, length(deg.range))
    ...
    for (i in 1:length(deg.range))
    {
      ...
      meantotRejection_impulsede2[i] <-mean(totRejection_impulsede2[i, ], na.rm=TRUE)
      vartotRejection_impulsede2[i]<-sqrt(var(totRejection_impulsede2[i, ], na.rm=TRUE))
      meanPower_impulsede2[i]<-mean(Power_impulsede2[i, ], na.rm=TRUE)
      meanacc_impulsede2[i]<-mean(acc_impulsede2[i, ], na.rm=TRUE)
      meanpre_impulsede2[i]<-mean(pre_impulsede2[i, ], na.rm=TRUE)
      meanrecall_impulsede2[i]<-mean(recall_impulsede2[i, ], na.rm=TRUE)
      meanspec_impulsede2[i]<-mean(spec_impulsede2[i, ], na.rm=TRUE)
      meanf1_impulsede2[i]<-mean(f1_impulsede2[i, ], na.rm=TRUE)
      meanFNR_impulsede2[i]<-mean(FNR_impulsede2[i, ], na.rm=TRUE)
      meanFDR_impulsede2[i]<-mean(FDR_impulsede2[i, ], na.rm=TRUE)
      meantp_impulsede2[i]<-mean(tp_impulsede2[i, ], na.rm=TRUE)
      meantn_impulsede2[i]<-mean(tn_impulsede2[i, ], na.rm=TRUE)
      meanfn_impulsede2[i]<-mean(fn_impulsede2[i, ], na.rm=TRUE)
      meanfp_impulsede2[i]<-mean(fp_impulsede2[i, ], na.rm=TRUE)
      meanTPR_impulsede2[i]<-mean(TPR_impulsede2[i, ], na.rm=TRUE)
      meanTNR_impulsede2[i]<-mean(TNR_impulsede2[i, ], na.rm=TRUE)
      
    }
    
    #cbind
    print(cbind(meanacc_ks, meanacc_edger,  meanacc_deseq2,meanacc_2masigpro,meanacc_splinetc, meanacc_funpat, meanacc_impulsede2))
    print(cbind(meanpre_ks, meanpre_edger,  meanpre_deseq2,meanpre_2masigpro,meanpre_splinetc, meanpre_funpat, meanpre_impulsede2))
    print(cbind(meanrecall_ks, meanrecall_edger,  meanrecall_deseq2,meanrecall_2masigpro, meanrecall_splinetc, meanrecall_funpat, meanrecall_impulsede2))
    print(cbind(meanspec_ks, meanspec_edger,  meanspec_deseq2,meanspec_2masigpro,meanspec_splinetc, meanspec_funpat, meanspec_impulsede2))
    print(cbind(meanf1_ks, meanf1_edger,  meanf1_deseq2,meanf1_2masigpro,meanf1_splinetc, meanf1_funpat, meanf1_impulsede2))
    print(cbind(meanFNR_ks, meanFNR_edger, meanFNR_deseq2,meanFNR_2masigpro,meanFNR_splinetc, meanFNR_funpat, meanFNR_impulsede2))
    print(cbind(meanFDR_ks, meanFDR_edger,  meanFDR_deseq2,meanFDR_2masigpro,meanFDR_splinetc, meanFDR_funpat, meanFDR_impulsede2))
    print(cbind(meantp_ks, meantp_edger, meantp_deseq2,meantp_2masigpro,meantp_splinetc, meantp_funpat, meantp_impulsede2))
    print(cbind(meantn_ks, meantn_edger, meantn_deseq2,meantn_2masigpro, meantn_splinetc, meantn_funpat, meantn_impulsede2))
    print(cbind(meanfn_ks, meanfn_edger,  meanfn_deseq2,meanfn_2masigpro,meanfn_splinetc, meanfn_funpat, meanfn_impulsede2))
    print(cbind(meanfp_ks, meanfp_edger,  meanfp_deseq2,meanfp_2masigpro, meanfp_splinetc, meanfp_funpat, meanfp_impulsede2))
    print(cbind(meanTPR_ks, meanTPR_edger,  meanTPR_deseq2,meanTPR_2masigpro, meanTPR_splinetc, meanTPR_funpat, meanTPR_impulsede2))
    print(cbind(meanTNR_ks, meanTNR_edger,  meanTNR_deseq2,meanTNR_2masigpro, meanTNR_splinetc, meanTNR_funpat, meanTNR_impulsede2))
    print(cbind(meantotRejection_ks, meantotRejection_edger,  meantotRejection_deseq2,meantotRejection_2masigpro, meantotRejection_splinetc, meantotRejection_funpat, meantotRejection_impulsede2))
    print(cbind(meanPower_ks, meanPower_edger, meanPower_deseq2,meanPower_2masigpro, meanPower_splinetc, meanPower_funpat, meanPower_impulsede2))
    print(cbind(vartotRejection_ks, vartotRejection_edger, vartotRejection_deseq2,vartotRejection_2masigpro, vartotRejection_splinetc, vartotRejection_funpat, vartotRejection_impulsede2))
    
    meanacc<- cbind(meanacc_ks, meanacc_edger, meanacc_deseq2,meanacc_2masigpro,meanacc_splinetc, meanacc_funpat, meanacc_impulsede2)
    meanpre<-cbind(meanpre_ks, meanpre_edger,  meanpre_deseq2,meanpre_2masigpro, meanpre_splinetc, meanpre_funpat, meanpre_impulsede2)
    meanrecall<-cbind(meanrecall_ks, meanrecall_edger,  meanrecall_deseq2,meanrecall_2masigpro,meanrecall_splinetc, meanrecall_funpat, meanrecall_impulsede2)
    meanspec <-cbind(meanspec_ks, meanspec_edger, meanspec_deseq2,meanspec_2masigpro,meanspec_splinetc, meanspec_funpat, meanspec_impulsede2)
    meanf1 <-cbind(meanf1_ks, meanf1_edger, meanf1_deseq2, meanf1_2masigpro,  meanf1_splinetc, meanf1_funpat, meanf1_impulsede2)
    meanFNR <- cbind(meanFNR_ks, meanFNR_edger, meanFNR_deseq2,meanFNR_2masigpro,meanFNR_splinetc, meanFNR_funpat, meanFNR_impulsede2)
    meanFDR <- cbind(meanFDR_ks, meanFDR_edger,  meanFDR_deseq2,meanFDR_2masigpro, meanFDR_splinetc, meanFDR_funpat, meanFDR_impulsede2)
    meantp <- cbind(meantp_ks, meantp_edger, meantp_deseq2,meantp_2masigpro, meantp_splinetc, meantp_funpat, meantp_impulsede2)
    meantn <- cbind(meantn_ks, meantn_edger,  meantn_deseq2,meantn_2masigpro,meantn_splinetc, meantn_funpat, meantn_impulsede2)
    meanfn <- cbind(meanfn_ks, meanfn_edger,  meanfn_deseq2,meanfn_2masigpro, meanfn_splinetc, meanfn_funpat, meanfn_impulsede2)
    meanfp <- cbind(meanfp_ks, meanfp_edger,  meanfp_deseq2,meanfp_2masigpro, meanfp_splinetc, meanfp_funpat, meanfp_impulsede2)
    meanTPR <- cbind(meanTPR_ks, meanTPR_edger,  meanTPR_deseq2,meanTPR_2masigpro, meanTPR_splinetc, meanTPR_funpat, meanTPR_impulsede2)
    meanTNR <- cbind(meanTNR_ks, meanTNR_edger,  meanTNR_deseq2,meanTNR_2masigpro, meanTNR_splinetc, meanTNR_funpat, meanTNR_impulsede2)
    meantotRejection <- cbind(meantotRejection_ks, meantotRejection_edger,  meantotRejection_deseq2,meantotRejection_2masigpro,  meantotRejection_splinetc, meantotRejection_funpat, meantotRejection_impulsede2)
    meanPower <- cbind(meanPower_ks, meanPower_edger,  meanPower_deseq2,meanPower_2masigpro, meanPower_splinetc, meanPower_funpat, meanPower_impulsede2)
    vartotRejection <- cbind(vartotRejection_ks,vartotRejection_edger,vartotRejection_deseq2,vartotRejection_2masigpro,vartotRejection_splinetc,vartotRejection_funpat,vartotRejection_impulsede2)
    ...
    boxplot(meanf1, col=c("red","blue","orange", "darkgreen","grey","cyan","purple"), xlab=NULL, ylab="F1 score", las=2)
    boxplot(meanPower,col=c("red","blue","orange", "darkgreen","grey","cyan","purple"), xlab=NULL, ylab="Power", las=2)
    boxplot(meanFDR, col=c("red","blue","orange", "darkgreen","grey","cyan","purple"), xlab=NULL, ylab="False Discovery Rate", las=2)
    boxplot(vartotRejection,col=c("red","blue","orange", "darkgreen","grey","cyan","purple"), xlab=NULL, ylab="SD of total discoveries", las=2)
