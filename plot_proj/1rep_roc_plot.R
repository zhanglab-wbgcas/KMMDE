...
for(rep_num in replicate.range){
  ...
  result$Methods <- factor(result$Methods,
                           levels = c("KMMDE", "edgeR","DESeq2","maSigPro","splineTC", "Funpat"))  #"maSigPro","splineTC",
  
  g <- ggplot(result, aes(d = DEG, m = ifelse(Methods == "KMMDE", 1-value, value), colour = Methods)) + scale_color_manual(values =c("red","blue","orange", "darkgreen","grey","cyan")) 
